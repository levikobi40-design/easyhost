...
function applyAuth(token, tenantId, role, loginSuccess) {
  ...
  // Decode JWT to persist email in the user store so Sidebar admin-email check works
  // even before a full-page re-render re-reads the token.
  try {
    const payload = parseJwtPayload(token);
    const emailFromToken = (payload.email || '').toLowerCase().trim();
    if (emailFromToken) {
      useStore.getState().setUser({ email: emailFromToken });
    }
  } catch (_) { /* ignore — JWT may be malformed during tests */ }
  try {
