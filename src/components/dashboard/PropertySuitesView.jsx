...
function PropertySuitesView({ suites, onAddSuite }) {
  ...
  return (
    <div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100" dir={dir}>
      <h3 className="text-2xl font-black text-gray-900 mb-6">{t('propertiesPage.suitesTitle')}</h3>

      <div className="space-y-4">
        {list.length > 0 ? (
          list.map((suite) => {
            const meta = [
              t('propertiesPage.suitesRooms', { count: suite.bedrooms ?? suite.rooms ?? 1 }),
              t('propertiesPage.suitesGuestsMax', { count: suite.guests ?? suite.max_guests ?? 2 }),
              suite.beds != null && suite.beds > 0
                ? t('propertiesPage.suitesBeds', { count: suite.beds })
                : null,
              suite.bathrooms != null && suite.bathrooms > 0
                ? t('propertiesPage.suitesBaths', { count: suite.bathrooms })
                : null,
            ].filter(Boolean).join(' • ');
            return (
              <div
              ...
              >
                ...
                <div className={dir === 'rtl' ? 'text-left' : 'text-right'}>
                  ...
                  <p className="text-[10px] text-gray-400 font-bold uppercase">{t('propertiesPage.suitesPerNight')}</p>
                </div>
              </div>
            );
          })
        ) : (
          <div className="py-8 text-center text-gray-400">{t('propertiesPage.suitesEmpty')}</div>
        )}
      </div>
