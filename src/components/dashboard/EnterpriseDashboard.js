...
const EnterpriseDashboard = () => {
  ...
  // Run once on mount — bootstrapOperationalData is internally deduplicated so
  // multiple components calling it concurrently produce only one network request.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      refreshPropertiesRef.current(true, true);
