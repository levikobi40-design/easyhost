...
export default function PremiumDashboard() {
  ...
  // Keep latest context functions in refs so loadDashboardData never needs them as
  // useCallback dependencies — prevents the fetch→state→re-render→fetch infinite loop.
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (loadId !== loadRef.current) return undefined;
      setSummary((prev) => prev ?? { revenue: '0', active_tasks_count: 0, status: 'Unavailable' });
      setStats((prev) => prev ?? {
        total_properties: 0,
  ...
  /** Re-sync when properties explicitly change — full reload needed for property count / summary. */
  useEffect(() => {
    const onPropertiesRefresh = () => {
      loadDashboardData();
    };
    window.addEventListener('properties-refresh', onPropertiesRefresh);
    return () => {
      window.removeEventListener('properties-refresh', onPropertiesRefresh);
    };
  }, [loadDashboardData]);

  /** Task board events — only refresh stats, NOT properties/full-dashboard.
  ...
  useEffect(() => {
    ...
    window.addEventListener('maya-refresh-tasks', onTasksRefresh);
    return () => {
      window.removeEventListener('maya-refresh-tasks', onTasksRefresh);
    };
  }, []);
  ...
  }, []); // fetchTasksRef is a ref — always current, no dep needed
  ...
  const handleToggleStatus = useCallback(async (taskId, newStatus, task) => {
    try {
      ...
      patchTaskInMissionRef.current(taskId, (t) => ({ ...t, status: newStatus }));
      const staffName = (task?.staff_name || task?.staffName || 'העובד').trim() || 'העובד';
      if (newStatus === 'Seen' || newStatus === 'In_Progress') {
        ...
        toggleMayaChat(true);
        // Refresh stats + task list so the dashboard counts update without a full reload
        window.dispatchEvent(new Event('maya-refresh-tasks'));
        try {
          const [freshTasks, freshStats] = await Promise.all([
            fetchTasksRef.current({ fullList: true }),
            getStatsSummary(),
          ]);
          if (freshStats) setStats(freshStats);
          if (freshTasks) { /* MissionContext already applied the update */ }
        } catch (_) { /* non-critical — board already patched optimistically */ }
      }
    } catch (e) {
