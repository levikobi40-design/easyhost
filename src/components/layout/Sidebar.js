...
/** Decode the email claim from a JWT without verifying the signature (UI-only). */
function _emailFromJwt(token) {
  try {
...
const Sidebar = ({ activeView, setActiveView }) => {
  ...
  // Force admin tier for hard-coded admin emails regardless of DB role.
  // Falls back to checking user.email from the store if the JWT hasn't been
  // decoded yet (e.g. immediately after login before re-render).
